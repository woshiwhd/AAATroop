...
namespace Script.Core
{
    ...
    public class Main : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            else GameLog.LogError("Main: Btn1 未在 Inspector 中赋值");

            if (Btn2 != null) Btn2.onClick.AddListener(OnBtn2Click);
            else GameLog.LogError("Main: Btn2 未在 Inspector 中赋值");
        ...
        private void ValidateSceneStructure()
        {
            ...
            if (GridRoot == null)
            {
                ...
                {
                    GridRoot = found;
                    GameLog.Log("Main: 自动找到 Grid 节点并赋值到 GridRoot。");
                }
                else
                {
                    GameLog.LogWarning("Main: GridRoot 未设置（可选）。建议在场景中放置一个名为 'Grid' 的 GameObject，并将 Tilemap 放在它下面。");
                }
            }
            ...
            if (GridRoot != null)
            {
                ...
                if (gridComp == null) GameLog.LogWarning("Main: GridRoot 未包含 Grid 组件，建议在 GridRoot 上添加 Grid 组件以便使用 Tilemap。");
                ...
                if (GroundTilemap == null)
                {
                    ...
                    {
                        GroundTilemap = tm;
                        GameLog.Log("Main: 在 Grid 下找到 Tilemap 并自动赋值给 GroundTilemap（建议命名为 Tilemap_Ground）。");
                    }
                    else
                    {
                        GameLog.LogWarning("Main: 在 GridRoot 下未找到 Tilemap 子节点，建议将 Tilemap (Tilemap_Ground / Tilemap_Blocking) 放在 GridRoot 下。");
                    }
                }
                ...
                if (BlockingTilemap == null)
                {
                    ...
                    foreach (var c in children)
                    {
                        ...
                        {
                            BlockingTilemap = c;
                            GameLog.Log("Main: 在 Grid 下找到可能的阻挡 Tilemap 并赋值给 BlockingTilemap。");
                            break;
                        }
                    }
                }
            ...
            if (EntitiesRoot == null)
            {
                ...
                {
                    EntitiesRoot = ent;
                    GameLog.Log("Main: 自动找到 Entities 节点并赋值到 EntitiesRoot。");
                }
            }
            ...
            if (PlayerObject == null)
            {
                ...
                {
                    PlayerObject = found;
                    GameLog.Log("Main: 自动找到 Player 并赋值到 PlayerObject。");
                }
                else
                {
                    GameLog.LogWarning("Main: 未找到 Player 节点（可选）。建议在 Entities 下创建名为 'Player' 的 GameObject 并添加 PlayerController。");
                }
            }

            // 输出一些最终建议和校验提示，帮助快速定位问题
            if (GroundTilemap == null) GameLog.LogWarning("Main: GroundTilemap 仍未设置，运行时依赖 Tilemap 的管理脚本可能无法正常工作。请在 Inspector 中手动设置 GroundTilemap 或确保场景中存在 Tilemap。");
            if (PlayerObject == null) GameLog.LogWarning("Main: PlayerObject 未设置。若需要玩家控制或运行时实体，请创建并设置 PlayerObject。");
        }

        private void OnBtn1Click()
        {
            if (Txt1 != null) Txt1.text = "Button 1 was clicked!";
            GameLog.Log("Btn1 Clicked");
        }
        private void OnBtn2Click()
        {
            if (Txt1 != null) Txt1.text = "Button 2 was clicked!";
            GameLog.Log("Btn2 Clicked");
        }
